...
import gensim.downloader # torchtext is unstable, use gensim instead
...
def load_data(args):
    ...
    w_ntokens = tokenizer.get_vocab_size() # [PAD] is already included when creating the tokenizer
    vars(args)['w_ntokens'] = w_ntokens

    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=False,
                              collate_fn=lambda b, tokenizer=tokenizer: collate_batch(b, args)) # shuffle=False will cause error
    test_loader = DataLoader(test_data, batch_size=40, shuffle=False,
                             collate_fn=lambda b, tokenizer=args.tokenizer: collate_batch(b, args))
    val_loader = None
    if val_data:
        val_loader = DataLoader(val_data, batch_size=40, shuffle=False,
            collate_fn=lambda b, tokenizer=tokenizer: collate_batch(b, args))
...
def collate_batch(batch, args):
    ...
    seq = [item.strip() for item in batch]
    seq_ids, seq_lengths, _ = _tokenize_words(seq, tokenizer)
...
def init_model(args):
    ...
    # Model
    model = RNN_padded(args.model_type, args.w_ntokens, args.w_emsize, args.nhid, args.nlayers, args).to(args.device)
    return model
...
def train(model, train_loader, criterion, optimizer, scheduler, epoch, args):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # Forward
        try:
            output, hidden = model(words, words_len, hidden)
        except Exception:
            # Print debug info
