...
from models import TransformerModel_padded, generate_square_subsequent_mask
...
def load_data(args):
    ...
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=False,
                              collate_fn=lambda b, args=args: collate_batch(b, args)) # shuffle=False will cause error
    test_loader = DataLoader(test_data, batch_size=40, shuffle=False,
                             collate_fn=lambda b, args=args: collate_batch(b, args))
    val_loader = None
    if val_data:
...
def collate_batch(batch, args):
    ...
    seq_ids, seq_lengths, pad_mask = _tokenize_words(seq, tokenizer)

    return seq_ids, seq_lengths, pad_mask
...
def init_model(args):
    ...
    model = TransformerModel_padded(args).to(args.device)
...
def train(model, train_loader, criterion, optimizer, scheduler, epoch, args):
    ...
    for step, batch in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        try:
            output = model(words, words_attn_mask, words_pad_mask)
        except RuntimeError:
            raise
...
def main(args):
    ...
    for epoch in range(1, epochs+1):
        ...
        val_loss = evaluate(model, val_loader, criterion, args)
