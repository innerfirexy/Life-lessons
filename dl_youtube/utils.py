...
class YougetDownloadTask(Task):
    command = "you-get"

    def __init__(self, url: str, path: str, args: Namespace):
        super(YougetDownloadTask, self).__init__()
        self.args = args
        self.n_trials = 0
        if re.search(r"(?<=\?v\=).+", url) is not None:  # Youtube
            self.video_id = re.search(r"(?<=\?v\=).+", url).group(0)
        elif re.search(r"(?<=video/).+(?=/)", url) is not None:  # Bilibili
            self.video_id = re.search(r"(?<=video/).+(?=/)", url).group(0)
        else:
            self.video_id = url
        path += "/" + self.video_id + "/"
        ...
        if args.cookies is not None:
            self.command += f" --cookies {args.cookies}"
        self.command += f" --output-dir {path}"
        # self.command += f" --output-filename {self.video_id}"
        self.command += f" {url}"

    def execute(self):
        ret = subprocess.run(self.command)
        if ret.returncode == 0:
            self.set_done()
            self.status = "success"
        else:
    ...
    def create_next_task(self):
        try:
            parse_task = ParseTask(
                input_file=self.downloaded_video_file,
                sample_interval=self.args.sample_interval,
                batch_mode=self.args.batch_mode,
                batch_size=self.args.batch_size,
                delete_after_done=self.args.delete_after_done,
...
class DownloadTask(Task):
    ...
    def __init__(self, download_path: str, url: str, args: Namespace):
        ...
        if re.search(r"(?<=\?v\=).+", url) is None:
            ...
            self.video_id = re.search(r"(?<=\?v\=).+", url).group(0)
        self.downloaded_video_file = os.path.join(
            self.download_path, self.video_id + ".mp4"
        )
        self.n_trials = 0
        ...
        try:
            ...
            if self.batch_mode:
                ...
                while video_capture.isOpened():
                    ...
                    if (
                        len(frames) == self.batch_size
                        or current_frame_index == total_frame_count - 1
                    ):  # Last frame
                ...
                number_of_faces_in_frames = []
                pbar = tqdm(
                    total=total_frame_count // self.sample_interval,
                    desc=f"Worker-{pid}",
                    position=pid,
                    leave=False,
                )
